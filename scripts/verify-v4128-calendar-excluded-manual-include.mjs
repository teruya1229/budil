/**
 * Budil v4.12.22 - excluded calendar candidates can be manually included as work orders.
 */
...
assert(indexHtml.includes('js/calendar-candidate-brain.js?v=4.13.18'), 'calendar-candidate cache buster should be v4.13.18');
...
assert(storageJs.includes("BUDIL_VERSION: 'v4.13.18'"), 'storage key namespace unchanged (version only)');
...
assert(statusMd.includes('verify-current') || handoffMd.includes('verify-current'), 'v4.12.25 verify-current ops retained');

console.log('\nAll v4.12.22 calendar-excluded-manual-include checks passed.');
