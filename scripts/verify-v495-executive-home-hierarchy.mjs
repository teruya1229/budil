/**
 * Budil v4.9.8 executive home information hierarchy verification.
 */
...
console.log('== v4.9.8 executive home hierarchy ==');
...
assert(indexHtml.includes('exec-home-section-summary'), 'summary section should exist');
...
const opsIdx = indexHtml.indexOf('exec-home-ops-flow');
const summaryIdx = indexHtml.indexOf('exec-home-section-summary');
...
assert(priorityIdx > 0 && opsIdx > priorityIdx, 'priority action should precede ops flow');
assert(summaryIdx > 0 && profitIdx > summaryIdx && monthlyIdx > profitIdx, 'summary → profit → monthly order');
...
assert(appJs.includes('exec-home-priority-action-lead'), 'priority action should have short lead text');
assert(appJs.includes("renderMonthlyClosingCheck('revenue-monthly-closing-check')"), 'revenue monthly closing full mode should remain');
...
console.log('All v4.9.8 executive home hierarchy checks passed.');
