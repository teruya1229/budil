...
assert(indexHtml.includes('js/storage.js?v=4.13.18'), 'storage cache buster should be v4.13.18');
...
assert(currentRunner.includes("EXPECTED_VERSION = 'v4.13.18'"), 'verify-current EXPECTED_VERSION should be v4.13.18');
...
console.log('== save path updates same id ==');
{
  ...
  const body = fn[0];
  assert(body.includes('Storage.updateRevenueRecord(id, confirmationSnapshot.payload)'), 'edit save must update existing record with the displayed snapshot');
  assert(body.includes('Storage.addRevenueRecord(confirmationSnapshot.payload)'), 'new save path may add the displayed snapshot');
