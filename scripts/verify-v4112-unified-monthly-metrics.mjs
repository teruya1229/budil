/**
 * Budil v4.11.2 - unified monthly revenue/profit metrics verification.
 */
...
console.log('== v4.11.2 unified-monthly-metrics ==');
...
assert(indexHtml.includes('js/revenue-brain.js?v=4.11.2'), 'revenue-brain cache buster should be v4.11.2');
assert(indexHtml.includes('js/executive-brain.js?v=4.11.2'), 'executive-brain cache buster should be v4.11.2');
...
assert(statusMd.includes('v4.11.2'), 'status.md should document v4.11.2');
assert(handoffMd.includes('v4.11.2'), 'handoff.md should document v4.11.2');
assert(decisionLog.includes('v4.11.2'), 'decision-log.md should record v4.11.2');
...
assert(revenueBrainJs.includes('plannedRevenue = confirmedRevenue + plannedAdditionalRevenue'), 'plannedRevenue should avoid double counting');
assert(revenueBrainJs.includes('Math.max(0, confirmedRevenue - paidAmount)'), 'unpaidAmount should use confirmed revenue');
assert(revenueBrainJs.includes('monthlyTarget - plannedRevenue'), 'remainingToTarget should use plannedRevenue');
...
assert(appJs.includes('今月予定売上'), 'executive home should show 今月予定売上');
assert(appJs.includes('今月予定利益'), 'executive home should show 今月予定利益');

console.log('== untouched files ==');
assert(!css.includes('v4.11.2'), 'css must not change for v4.11.2');
assert(!receptionJs.includes('buildSharedMonthlyMetrics'), 'reception-brain must not change for v4.11.2');
assert(!documentsJs.includes('buildSharedMonthlyMetrics'), 'documents-brain must not change for v4.11.2');
assert(!followJs.includes('buildSharedMonthlyMetrics'), 'follow-up-brain must not change for v4.11.2');
...
console.log('== numeric definition tests ==');
{
  ...
  const result = runInContext(`(() => {
    ...
    const records = [
      { id: 'r1', workDate: '2026-07-03', amount: 25000, status: '確定', paymentStatus: 'paid' },
    ...
    const workOrders = [
      { id: 'wo1', scheduledDate: '2026-07-07', estimateAmount: 32670, status: 'tentative' },
  ...
  assert(result.shared.plannedRevenue === 57670, `plannedRevenue should be 57670, got ${result.shared.plannedRevenue}`);
  ...
  assert(result.shared.plannedProfit === 54670, `plannedProfit should be 54670, got ${result.shared.plannedProfit}`);
  ...
  assert(result.execSection.plannedRevenue === result.shared.plannedRevenue, 'executive and shared plannedRevenue must match');
  ...
  assert(result.execSection.plannedProfit === result.shared.plannedProfit, 'executive and shared plannedProfit must match');
  assert(result.execSection.confirmedProfit === result.shared.confirmedProfit, 'executive and shared confirmedProfit must match');
  assert(result.execSection.monthRevenueLabel === '今月予定売上', 'executive label should be 今月予定売上');
}
...
console.log('\nAll v4.11.2 unified-monthly-metrics checks passed.');
