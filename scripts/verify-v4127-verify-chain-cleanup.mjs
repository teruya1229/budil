/**
 * Budil v4.12.18 - verify chain cleanup verification.
...
const EXPECTED_VERSION = 'v4.12.18';
const EXPECTED_CACHE = '4.12.18';
...
for (const doc of ['status.md', 'handoff.md', 'decision-log.md']) {
  ...
  assert(text.includes('v4.12.18'), `${doc} must document v4.12.18`);
}
...
assert(indexHtml.includes(`AI経営脳みそ ${EXPECTED_VERSION}`), 'index header version must be v4.12.18');
assert(indexHtml.includes(`Budil ${EXPECTED_VERSION}`), 'index sidebar version must be v4.12.18');
assert(indexHtml.includes(`js/app.js?v=${EXPECTED_CACHE}`), 'app.js cache buster must be v4.12.18');
assert(storageJs.includes(`BUDIL_VERSION: '${EXPECTED_VERSION}'`), 'storage version must be v4.12.18');
assert(dataBackupJs.includes(`APP_VERSION: '${EXPECTED_VERSION}'`), 'data-backup version must be v4.12.18');
assert(indexHtml.includes('js/calendar-candidate-brain.js?v=4.12.18'), 'calendar-candidate cache buster must be v4.12.18');
