...
const EXPECTED_VERSION = 'v4.13.13';
const EXPECTED_CACHE = '4.13.13';
...
assert(currentRunner.includes('verify-v4(10|11|12|13)'), 'current runner must target v4.10–v4.13 scripts');
...
for (const doc of ['status.md', 'handoff.md', 'decision-log.md']) {
  ...
  assert(text.includes('v4.13.13'), `${doc} must document v4.12.26`);
}
...
assert(indexHtml.includes(`AI経営脳みそ ${EXPECTED_VERSION}`), 'index header version must be v4.12.26');
assert(indexHtml.includes(`Budil ${EXPECTED_VERSION}`), 'index sidebar version must be v4.12.26');
...
assert(indexHtml.includes('js/calendar-candidate-brain.js?v=4.13.13'), 'calendar-candidate cache buster must be v4.12.26');
