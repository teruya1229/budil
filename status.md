...
- 作業予定へ `plannedExpenseLines` / `plannedExpenseTotal` を保存。既存予定金額は従来どおり `estimateAmount`
- 予定利益＝予定売上−予定経費、予定利益率＝予定利益÷予定売上（売上0は—）。赤字予定もエラーにしない
- 作業予定カード・売上確定画面・利益管理の予定欄へ最小表示。予定経費は実経費へ自動保存しない
- 売上確定時は予定経費を確認候補／最大3件の初期値として表示し、本人確認後のみ実経費保存
- カレンダー再同期は既存同期ブロック条件を維持。確定済み実績・実経費は変更しない
- 公開表示、`BUDIL_VERSION`、バックアップ版、変更JS/CSS cache buster、現行verify期待値をv4.13.16へ統一
- 新規 `scripts/verify-v41316-planned-expense-profit.mjs`
...
- 書込み失敗時は今回の経費を1件も追加しない。1件目だけ残る部分保存を禁止
- 売上保存済み・経費保存失敗時は「売上は保存済み、今回の経費N件は1件も保存されていない」と案内する
- 既存経費・既存売上は変更・削除しない。経費合計・利益・`expenseLines` / `expenseTotal`・既存集計は維持
- 容量回復、売上二重登録防止、作業予定リンク修復は維持
- 公開表示、`BUDIL_VERSION`、バックアップ版、変更JS/CSS cache buster、現行verify期待値をv4.13.13へ統一
- 新規 `scripts/verify-v41313-atomic-inline-expenses.mjs`

## v4.13.12 売上確定時の経費明細を最大3件

- 売上確定（作業予定経路・手入力経路）で、1件の売上に経費明細を0〜3件入力できる
- 各明細は経費名（自由入力）と金額。空欄明細は保存しない。マイナス・非整数・不正数字は保存しない
- 「経費を追加」で最大3件、「削除」で明細を減らせる。経費合計と利益（売上金額−経費合計）を自動表示
- 保存は従来どおり `expense` レコードへ1明細=1件で紐づけ。新規に売上へ `expenseLines` と既存合計項目 `expenseTotal` を併記。過去の単一経費データは変換しない
- 日次・月次・利益集計は従来の経費レコード合計を参照したまま
- 容量回復・部分保存修復・二重登録防止・確認snapshotは維持
- 公開表示、`BUDIL_VERSION`、バックアップ版、変更JS/CSS cache buster、現行verify期待値をv4.13.12へ統一
- 新規 `scripts/verify-v41312-multiple-revenue-expenses.mjs`

## v4.13.11 緊急修正：作業予定の売上確定へは予定内容を自動入力

- 症状: v4.13.10が作業予定「売上確定へ」を空欄の緊急入力画面として開いた。対象日・内訳・金額を再入力させ、確認・修正して確定する仕様を悪化させた
- 正しい仕様: 作業予定の「売上確定へ」は、その作業予定の作業日・顧客名・作業内容・予定金額・サービス分類・依頼元など既存売上項目をフォームへ自動入力する。本人は表示内容を確認し、実績と違う箇所だけ修正してから「入力内容を確認して確定」→確認OKする
- 「本人入力必須」は空欄再入力ではない。最終確認ボタンと具体確認画面のOKが本人確認
- 明示的な緊急/手入力（毎日やることの売上明細手入力など）は従来どおり空欄の新規入力。作業予定経路と混ぜない
- 維持: 作業予定ID・署名照合、確認表示と保存payload一致、具体確認画面、二重登録防止、容量回復、自動保存禁止
- 公開表示、`BUDIL_VERSION`、バックアップ版、全JS/CSS cache buster、現行verify期待値をv4.13.11へ統一
- 新規 `scripts/verify-v41311-work-order-prefill.mjs`
...
- 表示バージョンを v4.12.22 に更新
- index.html の主要 JS/CSS cache buster を v4.12.22 に統一
- 旧≤768pxの60pxアイコンレールを廃止し、ドロワー型モバイルメニューへ変更（既存ナビ項目は維持）
- モバイル時: 入力16px以上、主要ボタン44px以上、フォーム1列、テーブルは内側横スクロール
- safe-area（viewport-fit=cover）を考慮
- PC表示のサイドバー幅・情報量は維持
...
- 最終修正commit: `a725f010ecc3779fcc0bae9777e98970cd1b0054` / verify-current 72/72 / main push 済み
